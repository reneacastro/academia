...
function saveProfile() {
  ...
  var uid = btoa((nome+sobrenome).toLowerCase().replace(/\s+/g,'')).replace(/[^a-z0-9]/gi,'').slice(0,20) || 'user'+Date.now();
  currentUser = {
    uid: uid,
    name: nome.trim() + (sobrenome.trim() ? ' '+sobrenome.trim() : ''),
...
async function loadFromSheets() {
  ...
  var t = setTimeout(function(){ setSyncStatus('err','Sem conexão — dados locais'); }, 9000);
  try {
    ...
    // Merge perfil remoto (não sobrescreve campos locais não-nulos)
    if (pData && typeof pData==='object' && !pData.error) {
      currentUser = Object.assign({ altura:'', peso:'', objetivo:'' }, pData, currentUser);
      // mantém uid e name local, mas preenche campos extras se vieram do sheets
...
function showScreen(id) {
  ...
  if (id==='profile')   { /* preenche campos se já logado */ if(currentUser) openProfileFields(); }
  window.scrollTo(0,0);
}

function openProfileFields() {
  var parts = (currentUser.name||'').split(' ');
  var el;
  if ((el=document.getElementById('prf-nome')))      el.value = parts[0]||'';
  if ((el=document.getElementById('prf-sobrenome'))) el.value = parts.slice(1).join(' ')||'';
...
function renderHomeSavedWorkouts() {
  ...
  grid.innerHTML = sw.map(function(w){
    var cat=w.category||'l', col=catColors[cat]||'var(--blue)', bg=catBg[cat]||'#e8f0fe', ico=catEmoji[cat]||'⭐';
...
function renderSchedUI() {
  ...
  var cc = Object.assign({}, CAT_COLORS);
  var cb = Object.assign({}, CAT_BG);
  sw.forEach(function(w,i){ cc[String(w.id)]=wColors[i%wColors.length]; cb[String(w.id)]=wBgs[i%wBgs.length]; });
...
function applySchedSelStyle(sel,val,cc,cb){
  if(!cc){
    ...
    sw.forEach(function(w,i){ cc[String(w.id)]=wC[i%wC.length]; cb[String(w.id)]=wB[i%wB.length]; });
  }
  sel.style.background=cb[val]||'#e8f0fe'; sel.style.color=cc[val]||'var(--blue)'; sel.style.borderColor=cc[val]||'var(--blue)';
}
...
// ── Construção de URL de imagem usando wger.de (API pública de exercícios) ──
// Como o repo yuhonas usa nomes exatos de arquivo, montamos URL pelo slug
function buildImgUrl(slug) {
  if (!slug) return null;
  // O repo usa: https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/{Name}/0.jpg
  return GH + slug + '/0.jpg';
}
...
function renderExGrid(exIds,col,bg){
  ...
  exIds.forEach(function(eid,i){
    ...
    // monta HTML da imagem
    var imgHtml='';
    var url1=buildImgUrl(ex.p), url2=buildImgUrl(ex.f);
    if(url1||url2){
      var main=url1||url2, fb=url2||url1;
      imgHtml='<img class="img-a" src="'+main+'" alt="'+escAttr(ex.name)+'" '
        +'onerror="imgFallback(this,\''+escAttr(fb)+'\')">'
        +'<div class="img-fb"><span class="mi">fitness_center</span><span>Sem imagem</span></div>';
    } else {
      imgHtml='<div class="img-fb show"><span class="mi">fitness_center</span><span>Sem imagem</span></div>';
    }
    card.innerHTML=
      '<div class="img-wrap">'+imgHtml+'<div class="mpills">'+musclesPills+'</div></div>'
      +'<div class="ex-body">'
      ...
      +'</div>'
      +'<button class="check-btn" id="chk-'+eid+'" onclick="toggleEx(\''+eid+'\')"><span class="mi">radio_button_unchecked</span>Marcar como feito</button>'
      +'</div>';
    grid.appendChild(card);
  });
}

function imgFallback(img,fb){
  if(fb && img.src!==fb && !img.dataset.tried){ img.dataset.tried='1'; img.src=fb; return; }
  img.style.display='none';
  var f=img.parentElement.querySelector('.img-fb');
...
function selectCalWorkout(wid,wname,ev){
  ...
  var el=document.getElementById('cal-name-preview'); if(el) el.textContent='✔ '+wname+' selecionado';
...
function renderHist(){
  ...
  list.innerHTML=sorted.slice(0,30).map(function(e){
    return '<div class="hist-item"><span class="hist-date">'+fmtDate(e.date)+'</span>'
      ...
      +(e.calories?'<span class="hist-cal">🔥 '+e.calories+' kcal</span>':'')+'</div>';
  }).join('');
}
...
function setPeriod(days){
  ...
  dashEnd=end.toISOString().split('T')[0]; dashStart=start.toISOString().split('T')[0];
  document.querySelectorAll('.p-btn').forEach(function(b){b.classList.remove('on');});
...
function renderBarChart(data){
  ...
  var weeks={}; data.forEach(function(h){ var d=new Date(h.date+'T12:00:00'),wd=d.getDay(),mon=new Date(d); mon.setDate(d.getDate()-wd+1); var key=mon.toISOString().split('T')[0]; weeks[key]=(weeks[key]||0)+1; });
  var keys=Object.keys(weeks).sort(), vals=keys.map(function(k){return weeks[k];}), maxV=Math.max.apply(null,vals)||1;
...
function renderBank(){
  ...
  ['a','b','c','l'].forEach(function(cat){
    ...
    if(extras[cat].length)html+='<div class="bank-section"><div class="bank-sec-title">Exercícios Extras</div>'+renderBankList(extras[cat])+'</div>';
...
function updateSelCount(){
  ...
  var el=document.getElementById('sel-count'); if(el)el.textContent=n?n+' exercício(s) selecionado(s)':'';
}
...
function saveNewWorkout(){
  ...
  var idx=sw.findIndex(function(w){return String(w.id)===String(wid);}); if(idx>=0)sw[idx]=workout; else sw.push(workout);
  cacheSW();
  setSyncStatus('syncing','Salvando...');
