...
function startSavedWorkout(id) {
  ...
  (w.exercises||[]).forEach(function(eid){
    ...
    var var catEmoji = CAT_EMOJI[ex.cat] || '\U0001f3cb';
    var imgA = '<img id="img-'+eid+'" class="img-a" style="display:none;" alt="'+ex.name+'">'
             + '<div id="fb-'+eid+'" class="img-fb"><span style="font-size:36px;">'+catEmoji+'</span><span>'+ex.name+'</span></div>';
...
/* ── IMAGENS ── */
function applyExImage(eid, ex) {
  var imgEl = document.getElementById('img-'+eid);
  var fbEl  = document.getElementById('fb-'+eid);
  if (!imgEl || !ex || !ex.img) { showFb(eid); return; }
  var url1 = GH + ex.img + '/0.jpg';
  var url2 = ex.img2 ? GH + ex.img2 + '/0.jpg' : null;
  imgEl.onerror = function() {
    if (url2) {
      imgEl.onerror = function() { showFb(eid); };
      imgEl.src = url2;
    } else { showFb(eid); }
  };
  imgEl.src = url1;
...
function showFb(eid) {
  ...
  var im = document.getElementById('img-'+eid);
  if (fb) fb.style.display = 'flex';
  if (im) im.style.display = 'none';
}
function startAnim() {
  document.querySelectorAll('.ex-card').forEach(function(card) {
    var eid = card.id.replace('card-', '');
    var ex  = EX[eid]; if (!ex) return;
    applyExImage(eid, ex);
  });
}
